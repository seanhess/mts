...
// CREATE INSTANCES // 
    ...
    $file = $clientRequest->revertfile;
    $sourceName = $clientRequest->sourceName;
    $sourceFile = $clientRequest->sourceFile;
    $backupDir = $serverInfo->BackupDirectory;
    
    $path = $backupDir.$sourceName."/".$file;

    if ( copy($path, $sourceFile) )
        $serverResponse->setBoolean("reverted",true);
    else
        $serverResponse->throwError("The wiki at \"$sourceFile\" was not restored to the backup at \"$path\"");
